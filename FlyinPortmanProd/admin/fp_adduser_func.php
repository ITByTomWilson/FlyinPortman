...
function art_form_insertdata_display($form, $message){
    ...
    $artv_activeuser = art_request("activeuser", "");
    ...
    print "<td width=\"3\" class=\"formColumnData\">&nbsp;</td>\n";
    print "<td class=\"formColumnData\" align=\"left\">\n";
    print "<input class=\"textbox\" id=\"password\" name=\"password\" type=\"password\" value=\"" . $value . "\"  size=\"50\" maxlength=\"100\" >\n";
    print "</td>\n";
    print "</tr>\n";
    $value = $artv_userrank;
    if (($value == "") || ($value == null)) {
    ...
    if (($artsv_postback == null) || ($artsv_postback == "")){
        $value = "";
        $artv_activeuser = $value;
    }

    $control_name = "activeuser";
...
function art_insert_data() {
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        $sql = "INSERT INTO `fp_users`";
        $sql .= " (";
        $sql .= "fp_users.username"; 
        $sql .= ",fp_users.password"; 
        $sql .= ",fp_users.userRank"; 
        $sql .= ",fp_users.userActive"; 
        $sql .= ",fp_users.membershipDate"; 
        $sql .= ") ";
        $sql .= " VALUES "; 
        $sql .= " (";
