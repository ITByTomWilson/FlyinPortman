...
function art_form_insertdata_display($form, $message){
    ...
    print "<td width=\"3\" class=\"formColumnData\">&nbsp;</td>\n";
    print "<td class=\"formColumnData\" align=\"left\">\n";
    print "<input class=\"textbox\" id=\"fpnetworkid\" name=\"fpnetworkid\" type=\"text\" value=\"" . $value . "\"  size=\"10\" maxlength=\"10\" >\n";
    print "</td>\n";
    print "</tr>\n";
...
function art_insert_data() {
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        $sql = "INSERT INTO `fp_networks`";
        $sql .= " (";
        $sql .= "fp_networks.fpNetworkID"; 
        $sql .= ",fp_networks.networkName"; 
        $sql .= ",fp_networks.enabled"; 
        $sql .= ",fp_networks.sysDTStamp"; 
        $sql .= ") ";
        $sql .= " VALUES "; 
        $sql .= " (";
        $sql .=  art_quote_intval($artv_fpnetworkid); 
        $sql .= " , " .  art_quote_strval($artv_networkname); 
        $sql .= " , " .  art_quote_strval($artv_enabled); 
        $sql .= " , " .  art_quote_dateval($artv_sysdtstamp); 
       $sql .= ") ";
    }
